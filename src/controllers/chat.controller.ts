...
export const getConversationsByEmail = async (
  ...
  try {
    ...
    if (conversationData.rowCount === 0)
      return res.status(400).json("User haven't created conversations");
...
export const updateMessageInConversation = async (
  ...
  try {
    ...
    if (messageData.rowCount === 0)
      return res.status(400).json("Cannot find conversation");
    ...
    const updateData: QueryResult = await pool.query(updateQuery, [
      data.messages,
      id,
    ]);
